...
class AuthManager {
    ...
    getUserData() {
        const userData = localStorage.getItem(this.USER_DATA_KEY) || sessionStorage.getItem(this.USER_DATA_KEY);
        if (userData) {
            try {
    ...
    async updateUserProfile(profileData) {
        try {
            ...
            if (response && response.ok) {
                ...
                this.storeUserData(updatedUser, this.getToken(), !!localStorage.getItem(this.TOKEN_KEY));
                return data;
            }
        } catch (error) {
    ...
    async updateCartItem(title, quantity) {
        try {
            const response = await this.makeAuthenticatedRequest('/users/cart/update', {
                method: 'PUT',
    ...
    async removeFromCart(title) {
        try {
            const response = await this.makeAuthenticatedRequest('/users/cart/remove', {
                method: 'DELETE',
    ...
    async clearCart() {
        try {
            const response = await this.makeAuthenticatedRequest('/users/cart/clear', {
                method: 'POST'
            });
    ...
    async createOrderFromCart(orderData) {
        try {
            const response = await this.makeAuthenticatedRequest('/orders/from-cart', {
                method: 'POST',
    ...
    async getUserOrders(page = 1, limit = 10) {
        try {
            const response = await this.makeAuthenticatedRequest(`/orders?page=${page}&limit=${limit}`);
            if (response && response.ok) {
                const data = await response.json();
                return data;
            }
    ...
    async updateOrderStatus(orderId, status, notes = '') {
        try {
            const response = await this.makeAuthenticatedRequest(`/orders/${orderId}/status`, {
                method: 'PUT',
